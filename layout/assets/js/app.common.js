...
function resizeContent() {
    ...
    _work_area_height_ = $height - 180;
    /*
    if($('.dataTable_container').length ){
      $('.dataTable_container').height($height);
    }
    */
    if($('.full_height').length ){
      $('.full_height').height($height);
    }
}
...
function MessageBox(/* ,args */){
  ...
  var type       = "";
  var pageToLoad = "";
  var funcToExecute = function(){ console.log("Default callback in MessageBox();"); };
  ...
  if( typeof(args) == 'object' ){
      ...
      if( isset(args['callback']) == true){
        if( typeof(args['callback']) == 'function' ){
            funcToExecute = args['callback'];
        }
        else{
          console.log("MessageBox: No se ha enviado una función válida en el argumento 'callback'. Tipo de dato enviado:");
  ...
  if(pageToLoad == ""){
    ...
    console.log("MessageBox : Mostrando " + pageToLoad);
    BootstrapDialog.show({
        ...
        buttons: [
            {
                label: 'Cerrar',
                cssClass: 'btn-raised btn-success',
                action: function(dialogItself){
                    dialogItself.close();
                }
            },
            {
                label: 'Cerrar y actualizar',
                cssClass: 'btn-raised btn-primary',
                action: function(dialogItself){
                    dialogItself.close();
